...
window.__ModuleLoader__.load({
  ...
  const apply = (ctx) => {
    ...
    const CSS_RULES = [
      ...
      // v5.30: 波层几何（top/height/animation/opacity）改由 WAVE_VARIANTS 按档 inline 注入，
      // 这里只留波带公共骨架；右键切档标签用 .dshp-wvtip。
      '.dshp-wave{position:absolute;left:0;width:200%;display:block}',
      '.dshp-wvtip{position:absolute;left:50%;top:calc(100% + 8px);transform:translateX(-50%);padding:4px 10px;border-radius:9px;background:rgba(20,25,34,.9);border:1px solid rgba(255,255,255,.14);color:#dde6f0;font-size:10.5px;font-weight:600;letter-spacing:.06em;white-space:nowrap;pointer-events:none;animation:dshp-wvtip 1.8s ease both;z-index:2}',
      '@keyframes dshp-wvtip{0%{opacity:0;transform:translateX(-50%) translateY(-4px)}12%{opacity:1;transform:translateX(-50%) translateY(0)}78%{opacity:1}100%{opacity:0;transform:translateX(-50%) translateY(2px)}}',
...
// v5.17: 小数部分半号（用户裁定：大项目小数变化慢，降一级视觉权重）
    ...
    // v5.30 波浪三档测试台（用户裁定：先测 B → 再比 A/C → 三选一，右键水球循环切档）。
    // 波形骨架与 v5.10 同构：周期 30 单位、120 视窗宽 = 2 整周期，translateX(-50%) 无缝循环；
    // 振幅通过 viewBox 加高 + 波带加高放大（SVG 默认裁剪到视窗，crest 必须留在 viewBox 内）。
    // 屏上峰谷 = (2a/vbh) * h px；球内水体直径 80px。
    //   B 保守：2 层，峰谷 16px（振幅 ×1.4），淡层提浓；
    //   A 中档：3 层错速错相位（1 层反向 + 1 层倒相），峰谷 ≈18px（球径 22%）；
    //   C 激进：3 层，峰谷 ≈24px（球径 30%），浪尖偶尔贴数字。
    // 选定胜出档后：固化该档为唯一实现并移除测试台（本段 + wv 状态 + 右键 + wvtip）。
    ...
    const WAVE_VARIANTS = [
      { tag: 'B 保守档 · 2层 振幅×1.4 提浓', vbh: 36, layers: [
        { d: wavePath(12, 12, 36), top: -23, h: 24, s: 3.2, k: .8, a: .8, o: 1 },
        { d: wavePath(12, 12, 36), top: -18, h: 24, s: 5.1, rev: true, k: .6, a: .7, o: .8 },
      ] },
      { tag: 'A 三层大浪 · 中档 · 球径22%', vbh: 40, layers: [
        { d: wavePath(14, 14, 40), top: -25, h: 26, s: 3.1, k: .8, a: .85, o: 1 },
        { d: wavePath(14, 14, 40), top: -19, h: 26, s: 5.0, rev: true, k: .5, a: .7, o: .9 },
        { d: wavePath(14, 14, 40, true), top: -13, h: 26, s: 4.0, k: .3, a: .62, o: .85 },
      ] },
      { tag: 'C 三层大浪 · 激进 · 球径30%', vbh: 48, layers: [
        { d: wavePath(17, 17, 48), top: -33, h: 34, s: 3.0, k: .8, a: .85, o: 1 },
        { d: wavePath(17, 17, 48), top: -25, h: 34, s: 5.2, rev: true, k: .5, a: .7, o: .9 },
        { d: wavePath(17, 17, 48, true), top: -16, h: 34, s: 4.1, k: .3, a: .62, o: .85 },
      ] },
    ]
    ...
    const Panel = () => {
      ...
      // v5.30 波浪测试台：wv 当前档（默认 B，用户裁定先测 B）；wvTip 计数器驱动标签重播
      const [wv, setWv] = React.useState(() => {
        try { const v = parseInt(window.localStorage.getItem('dshp-wv'), 10); return (v >= 0 && v < WAVE_VARIANTS.length) ? v : 0 } catch (e) { return 0 }
      })
      const [wvTip, setWvTip] = React.useState(0)
      ...
      // v5.30: 右键水球循环切波浪档（B→A→C→B），标签浮 1.8s；localStorage 记档防刷新回退
      const onWaveMenu = (e) => {
        e.preventDefault()
        const n = (wv + 1) % WAVE_VARIANTS.length
        setWv(n); setWvTip((t) => t + 1)
        try { window.localStorage.setItem('dshp-wv', String(n)) } catch (err) {}
      }
      ...
      if (!open) {
        ...
        return h('div', {
          ...
          onPointerDown: onDown, onDoubleClick: onDbl, onContextMenu: onWaveMenu,
          title: (sys ? 'CPU ' + Math.round(sys.cpuPct) + '% · 内存 ' + Math.round(memPct) + '% · ' : '') + '总进度 ' + pct.toFixed(1) + '%' + (stale.length ? ' · ⚠ ' + stale.length + ' 项心跳中断' : '') + ' · 双击展开 · 拖动移动 · 右键切波浪档 · 蓝→绿→橘→红→紫黑',
        },
          h('div', { className: 'dshp-bublift' },
            ...
            h('div', { className: 'dshp-bubclip' },
              h('div', { className: 'dshp-water', style: { height: hgt + '%', background: rampRgba(hgt, .85) } },
                hgt > 0 && hgt < 100 ? WAVE_VARIANTS[wv].layers.map((L, i) => h('svg', {
                  key: 'wv' + i, className: 'dshp-wave', viewBox: '0 0 120 ' + WAVE_VARIANTS[wv].vbh, preserveAspectRatio: 'none',
                  style: { top: L.top + 'px', height: L.h + 'px', opacity: L.o, animation: 'dshp-wave ' + L.s + 's linear infinite' + (L.rev ? ' reverse' : '') },
                }, h('path', { d: L.d, style: { fill: rampLight(hgt, L.k, L.a) } }))) : null)),
            h('div', { className: 'dshp-fx dshp-fx-' + rot.fx, key: rot.t },
              ...
              wvTip > 0 ? h('div', { className: 'dshp-wvtip', key: 't' + wvTip }, WAVE_VARIANTS[wv].tag) : null)))
      }
