...
// v5.34: 浪层定稿 B 档（峰谷16px·深浅淡·近深远淡），测试台移除，冻结为唯一实现。
...
// v5.17: 小数部分半号（用户裁定：大项目小数变化慢，降一级视觉权重）
    ...
    // v5.34 浪层定稿（用户裁定：B 档）——右键切档测试台（v5.30）移除，冻结为唯一实现。
    // 定稿参数链：
    //   几何（B 档）：vbh 36 / a 12 / 波带 24px → 屏上峰谷 16px（v5.10 的 ×1.4），水体直径 80px；
    //   色阶（v5.31/32）：深=水体同色(k0) / 浅=+40% / 淡=+70%，全部 α.85 实色（表观色不随底层漂移）；
    //   层序（v5.33 近深远淡）：由远及近绘制 淡(4.2s 倒相) → 浅(5.1s 反向) → 深(3.4s)，
    //     近浪与水体无缝相接，α.85 透 15% 远浪成"深水半掩远波"纵深；
    //   结构（v5.32）：三层浪底边全部贴水位线，水下零平直边（波带垂水下会制造假层）。
    // 波形骨架与 v5.10 同构：周期 30 单位、120 视窗宽 = 2 整周期，translateX(-50%) 无缝循环；
    // 屏上峰谷 = (2a/vbh) * h px。层间去走样靠速度差 + 反向 + 倒相，不靠垂直错位。
    ...
    // 色阶三档（全部 α.85 实色）：k 提亮比例。t=0 即"深"（与水体同色）。
    const WAVE_TINTS = [
      { k: 0, a: .85, o: 1 },
      { k: .4, a: .85, o: 1 },
      { k: .7, a: .85, o: 1 },
          ...
          h('div', { className: 'dshp-bublift' },
            ...
            h('div', { className: 'dshp-bubclip' },
              h('div', { className: 'dshp-water', style: { height: hgt + '%', background: rampRgba(hgt, .85) } },
                hgt > 0 && hgt < 100 ? WAVE.layers.map((L, i) => {
                  const T = WAVE_TINTS[L.t]
                  return h('svg', {
                    key: 'wv' + i, className: 'dshp-wave', viewBox: '0 0 120 ' + WAVE.vbh, preserveAspectRatio: 'none',
                    style: { top: -WAVE.h + 'px', height: WAVE.h + 'px', opacity: T.o, animation: 'dshp-wave ' + L.s + 's linear infinite' + (L.rev ? ' reverse' : '') },
                  }, h('path', { d: wavePath(WAVE.a, WAVE.a, WAVE.vbh, L.inv), style: { fill: rampLight(hgt, T.k, T.a) } }))
                }) : null)),
