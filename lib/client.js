...
// v5.34: 浪层定稿 B 档（峰谷16px·深浅淡·近深远淡）；v5.35: 浪体合一——深浪贯穿球体即水体本体；
// v5.36: 三层浪高差——浅+2px、淡+5px；v5.37: 浪线均值锚定水位线；v5.38: 面板任务条逆向色带。
...
// v5.17: 小数部分半号（用户裁定：大项目小数变化慢，降一级视觉权重）
    ...
    // v5.38 详细进度条逆向色带（用户裁定：任务条颜色逆向水球总色带——起步紫黑 → 红 → 橘
    // → 绿 → 完成蓝，与总球（蓝→绿→橘→红→紫黑）互为镜像，面板观感更舒适）。
    // 运行态填充色 = 球色(100−p)；CPU/内存系统条保持"高负载=热色"语义不逆向。
    const fillStyle = (p, cls) => (
      cls === 'run'
        ? { width: p + '%', background: rampCss(100 - p) }
        : { width: p + '%' }
    )
    ...
    const WAVE = {
      ...
      // v5.36 层高差（用户裁定：无高度差则三层看不出区别）——整体抬升方向向上，
      // 浪带底浮在深色水体上方，不产生水下假层（v5.32 教训是带底垂到水线以下）。
      // 层间重叠区若用 α.85 会二次合成出第 4 种表观色（浅压淡≠浅压玻璃），三层全实色。
      layers: [
        { k: .7, a: 1, s: 4.2, inv: true, off: 5 },  // 淡（远）比浅高 3px
        { k: .4, a: 1, s: 5.1, rev: true, off: 2 },  // 浅（中）比深高 2px
      ],
      deep: { s: 3.4 },                               // 深（近）＝水体本体（k0 α1，off=0）
    }
    // 深浪专用 viewBox 高：波带段几何与薄浪完全同构，延长段只是纯填充
    const WAVE_DEEP_VBH = WAVE.vbh + Math.round(WAVE.deepExtra * WAVE.vbh / WAVE.h)
    // v5.37 感知水位锚定（用户裁定：80% 读作满球、38% 读作半球是 bug）——
    // 浪体合一后深浪即水体，眼睛改读浪线；而浪线整体悬在水位线上方（曲线占波带上段，
    // 均值 +16px = +20 个百分点），水位被读高。修法：浪带整体下移曲线振幅（16px），
    // 曲线在水位线 ±8px 振荡、均值=水位线，感知水位回归真实百分比。
    // 副作用自检：薄浪带底垂到水位线下方，但都在深浪填充之下（深浪最高 z 且 α1），
    // 不可见 → 不产生 v5.32 的水下假层。
    const WAVE_LIFT = Math.round((2 * WAVE.a / WAVE.vbh) * WAVE.h) // 曲线振幅 px（峰谷 16）
          ...
          h('div', { className: 'dshp-bublift' },
            ...
            h('div', { className: 'dshp-bubclip' },
              h('div', { className: 'dshp-water', style: { height: hgt + '%' } },
                // 远/中两层薄浪（淡、浅）：整体抬升 off px 形成层高差；只在 0<hgt<100 显示
                hgt > 0 && hgt < 100 ? WAVE.layers.map((L, i) => h('svg', {
                  key: 'wv' + i, className: 'dshp-wave', viewBox: '0 0 120 ' + WAVE.vbh, preserveAspectRatio: 'none',
                  style: { top: (-WAVE.h + WAVE_LIFT - (L.off || 0)) + 'px', height: WAVE.h + 'px', animation: 'dshp-wave ' + L.s + 's linear infinite' + (L.rev ? ' reverse' : '') },
                }, h('path', { d: wavePath(WAVE.a, WAVE.a, WAVE.vbh, L.inv), style: { fill: rampLight(hgt, L.k, L.a) } }))) : null,
                // 近层深浪（α1 实色）＝水体本体：波带拉高贯穿至球底，曲线均值锚定水位线
                hgt > 0 ? h('svg', {
                  key: 'wvd', className: 'dshp-wave', viewBox: '0 0 120 ' + WAVE_DEEP_VBH, preserveAspectRatio: 'none',
                  style: { top: (-WAVE.h + WAVE_LIFT) + 'px', height: (WAVE.h + WAVE.deepExtra) + 'px', animation: 'dshp-wave ' + WAVE.deep.s + 's linear infinite' },
